...
**Estado:** pendiente, evaluar en Hito 6. **Alternativas:** (a) sin navegador headless — validar solo el HTML/CSS/Deck Spec de forma estática (más simple, no captura problemas de layout real); (b) con navegador headless (Playwright/Puppeteer) para capturas y detección real de overflow/colisión visual. **Recomendación tentativa:** (b), porque el proyecto pone mucho peso en fidelidad visual y colisión del logo — una validación puramente estática no puede detectar overflow real de texto. **Riesgo:** añade una dependencia pesada; evaluar costo antes de comprometerse. **Momento:** decidir en Hito 6, no antes.
